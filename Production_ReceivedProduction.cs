...
namespace AB
{
    public partial class Production_ReceivedProduction : Form
    {
        ...
        private async void Production_ReceivedProduction_Load(object sender, EventArgs e)
        {
        ...
        public void loadData()
        {
            if (Login.jsonResult != null)
            {
                ...
                if (!token.Equals(""))
                {
                    ...
                    var request = new RestRequest("/api/production/rec_from_prod/get_all" + sDocStatus + sBranch + sWarehouse + sDate + sMode+ sFromTime + sToTime);
                    Console.WriteLine("/api/production/rec_from_prod/get_all" + sDocStatus + sBranch + sWarehouse + sDate + sMode + sFromTime + sToTime);
                                        ...
                                        if (x.Value.ToString() != "[]")
                                        {
                                            ...
                                            for (int i = 0; i < jsonArray.Count(); i++)
                                            {
                                                ...
                                                if (!string.IsNullOrEmpty(txtSearch.Text.ToString().Trim()))
                                                {
                                                    ...
                                                    {
                                                        dgv.Rows.Add(id, dtTransDate.ToString("yyyy-MM-dd HH:mm"), referenceNumber, docStatus, sapNumber, remarkss,docStatus.Equals("Open") ? "Close" : "");
                                                    }
                                                }
                                                else
                                                {
